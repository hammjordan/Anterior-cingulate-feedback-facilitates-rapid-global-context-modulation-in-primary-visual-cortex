clear all
close all
files={'120623_m1_pyrs_global_001_pre';
    '120623_m2_pyrs_global_001_pre';
    '120623_m3_pyrs_global_001_pre';
    '120623_m4_pyrs_global_001_pre';
    '012924_m1_pyrs_global_001_pre';
    '013024_m1_pyrs_global_002_pre';'013024_m1_pyrs_global_rev_002_pre';
    '013124_m1_pyrs_global_rev_002_pre';
    '020524_m1_pyrs_global_001_pre';
    '020524_m2_pyrs_global_001_pre';
    '013124_m1_pyrs_global_002_pre';
    '012924_m1_pyrs_global_rev_001_pre';
    '101523_m1_pyrs_global_rev_001_pre'
    };
mss=[1 2 3 4 5 6 6 7 8 9 7 5 10];
sxx=[2 2 2 2 1 2 2 2 1 1 2 1 1];

%during oddball phase, the 4's are the local oddball (e.g. 2), the 6's are the same orientation (e.g. 1) aand the 8's
%are the fifth orientation (e.g. 5)
...
maxnorm=0; %if 0, use mean of baseline. if 1, use min of baseline
exclloco=1; limtrials=6; rejectingdoubles=1;

for fil=1:size(files,1)

    load(strcat('CA_workspace_SORTED_',files{fil}))
   ...
   for limtrials=limtrials;  %kept to 6 trials to optimize comparison with oddball run
       if limtrials>0;
           counts=zeros(1,8);
    ...
    %part 2 limtrials (after locomotion exclude. need this too)
    for limtrials=limtrials; %dont need to do this for the 6 and8, but only 4
       if limtrials>0;
           %vecstim=vecstim1;
    ...
    trs=[4 6 8];
    numtrials(fil,4)=sum(vecstim==4);numtrials(fil,5)=sum(vecstim==6);numtrials(fil,6)=sum(vecstim==8); %check these to make sure there are at least 4 trials in each condition


    for cell=1:size(vecdat,1);
        celltmp=[];
        for ori=1
            celltmp(:,1)=squeeze(mean(vecdat(cell,:,and(ords==1,vecstim==2)),3));
        end
...
for cell=1:cellcount;
    if do_onsets==1;
    if or(and(inc_cntON(cell,1)>stdcut1, inc_OBON(cell,3)>stdcut1),or(inc_cntON(cell,1)>stdcut2, inc_OBON(cell,3)>stdcut2))
        incRDNT(1,cell)=1;
    end
    if or(and(inc_cntON(cell,2)>stdcut1, inc_OBON(cell,2)>stdcut1),or(inc_cntON(cell,2)>stdcut2, inc_OBON(cell,2)>stdcut2))
        incDEV1(1,cell)=1;
    end
    if or(and(inc_cntON(cell,3)>stdcut1, inc_OBON(cell,4)>stdcut1),or(inc_cntON(cell,3)>stdcut2, inc_OBON(cell,4)>stdcut2))
        incDEV2(1,cell)=1;
    end
    end
    if do_offsets==1;
    if or(and(inc_cntOFF(cell,1)>stdcut1, inc_OBOFF(cell,3)>stdcut1),or(inc_cntOFF(cell,1)>stdcut2, inc_OBOFF(cell,3)>stdcut2))
        incRDNT(1,cell)=1;
    end
    if or(and(inc_cntOFF(cell,2)>stdcut1, inc_OBOFF(cell,2)>stdcut1),or(inc_cntOFF(cell,2)>stdcut2, inc_OBOFF(cell,2)>stdcut2))
        incDEV1(1,cell)=1;
    end
    if or(and(inc_cntOFF(cell,3)>stdcut1, inc_OBOFF(cell,4)>stdcut1),or(inc_cntOFF(cell,3)>stdcut2, inc_OBOFF(cell,4)>stdcut2))
        incDEV2(1,cell)=1;
    end
    end
end
     ...
     figure(4); subplot(1,4,3); errorbar_groups([mean(tmp2) mean(tmp1)]',[std(tmp2)./sqrt(size(tmp1,1)-1) std(tmp1)./sqrt(size(tmp1,1)-1)]', 'bar_colors',[.5 .5 .5; 1 0 0]);
        ...
        for doLMEstats=doLMEstats;
            if doLMEstats==1;
                ...
                T = table(Value, categorical(Condition), categorical(Mouse), categorical(Sex), ...
                    categorical(FOV), categorical(Cell), ...
                    'VariableNames', {'Value', 'Condition', 'Mouse', 'Sex', 'FOVid', 'Cell'});

                lme = fitlme(T, 'Value ~ Condition + (1|Mouse) + (1|Mouse:FOVid) + (1|Sex) + (1|Mouse:Cell)');
   ...
   clear x;  tmp1=mean(OB(:,stimON),2); tmp2=mean(CT(:,stimON),2); aa=horzcat(tmp1,tmp2); x(1)=min(aa(:))*1.02; x(2)=max(aa(:))*1.02;
    ...
    for rasterpoot=1;
        ...
        for settingcolorscale=1;
            ...
            for z1=1:2;
                subplot(1,2,z1);
               % cb=caxis; if cb(1)<tmpmin; tmpmin=cb(1); end %comment out if wanting to scale by first raster
                if cb(2)>tmpmax; tmpmax=cb(2); end
            end
            for z1=1:2;
                subplot(1,2,z1);
                %cb=[tmpmin tmpmax]; %comment out if wanting to scale by first raster
       ...
       set(gcf,'Color','w');
         ...
         for doLMEstats=doLMEstats;
            if doLMEstats==1;
                ...
                anova_results = anova(lme, 'DFMethod', 'satterthwaite');
             %disp(strcat('OFFlocal rdnt global DD. LME F=', num2str(anova_results.FStat(2)), '. p=',  num2str(anova_results.pValue(2)),' df=',num2str(anova_results.DF2(2))))
...
for makepies=0;
    if makepies==1;
        figure;
        onlyB=and(incDEV1==1,(incRDNT+incDEV2)==0)
        onlyC=and(incDEV2==1,(incRDNT+incDEV1)==0)
        onlyA=and(incRDNT==1,(incDEV1+incDEV2)==0)
        BandC=and(and(incDEV2==1,incDEV1==1),incRDNT==0)
        BandA=and(and(incRDNT==1,incDEV1==1),incDEV2==0)
        CandA=and(and(incRDNT==1,incDEV2==1),incDEV1==0)
        allthree=and(and(incRDNT==1,incDEV2==1),incDEV1==1)
        any=or(or(incRDNT==1,incDEV2==1),incDEV1==1)
        pie(horzcat(mean(onlyA),mean(onlyB),mean(onlyC),mean(BandA),mean(CandA),mean(BandC),mean(allthree))./mean(any),...
            {strcat('onlyA',num2str(mean(onlyA)./mean(any)*100)),...
            strcat('onlyB',num2str(mean(onlyB)./mean(any)*100)), ...
            strcat('onlyC',num2str(mean(onlyC)./mean(any)*100)), ...
            strcat('AandB',num2str(mean(BandA)./mean(any)*100)), ...
            strcat('AandC',num2str(mean(CandA)./mean(any)*100)), ...
            strcat('BandC',num2str(mean(BandC)./mean(any)*100)), ...
            strcat('allthree',num2str(mean(allthree)./mean(any)*100))
                })



       
        ONincRDNT=zeros(1,cellcount); ONincDEV1=zeros(1,cellcount); ONincDEV2=zeros(1,cellcount);
        OFFincRDNT=zeros(1,cellcount); OFFincDEV1=zeros(1,cellcount); OFFincDEV2=zeros(1,cellcount);
       %inc_cntON=inc_OBON(:,2:4); inc_cntOFF=inc_OBOFF(:,2:4);  %just looking at oddball
       %  inc_OBON(:,2:4)=inc_cntON; inc_OBOFF(:,2:4)=inc_cntOFF; %just lookin at control 
        for cell=1:cellcount
                ...
                incON=(ONincRDNT+ONincDEV1+ONincDEV2); incON=(incON>0).*1;
                incOFF=(OFFincRDNT+OFFincDEV1+OFFincDEV2); incOFF=(incOFF>0).*1;
        
        figure;
        onlyON=and(incON==1,incOFF==0);
        onlyOFF=and(incOFF==1,incON==0);
        ONandOFF=and(incON==1,incOFF==1);
        any=or(incOFF==1,incON==1);
        pie(horzcat(mean(onlyON),mean(onlyOFF),mean(ONandOFF))./mean(any),...
            {strcat('onlyON',num2str(mean(onlyON)./mean(any)*100)),...
            strcat('onlyOFF',num2str(mean(onlyOFF)./mean(any)*100)), ...
            strcat('ONandOFF',num2str(mean(ONandOFF)./mean(any)*100))
                })
...
totalresponsivecells=362; %figure this out separately. 
%for these parts, make do_onsets and do_offsets both =1 (lines 255,256)
